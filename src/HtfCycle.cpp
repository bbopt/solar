...
HtfCycle::HtfCycle(double receiverTemp,
	double storageHeight, double storageDiameter, double insulationThickness,
	double exchangerExitTemp,
	Powerblock* powerblock,
	double apertureHeight, double apertureWidth,
	double receiverTubesDin, double receiverTubesThickness, int receiverNbTubes,
	double precision, int timeInterval)
  :

  //Mechanical components
  _centralReceiver(&_centralReceiverInlet, &_centralReceiverOutlet, apertureHeight, apertureWidth, insulationThickness, receiverTubesDin, receiverTubesThickness, receiverNbTubes),
  _hotStorage(&_centralReceiverOutlet, &_steamGeneratorInlet, storageHeight, storageDiameter, insulationThickness),
  _coldStorage(&_steamGeneratorOutlet, &_centralReceiverInlet, 1.2*storageHeight, storageDiameter, insulationThickness),
  _steamGenerator(&_steamGeneratorInlet, &_steamGeneratorOutlet, powerblock),

  //MoltenSalt conditions
  _centralReceiverInlet(exchangerExitTemp, P_ATM, 0.),
  _centralReceiverOutlet(receiverTemp, P_ATM, 0.),
  _steamGeneratorInlet(receiverTemp, P_ATM, 0.),
  _steamGeneratorOutlet(exchangerExitTemp, P_ATM, 0.), 

  //Design parameters
  _centralReceiverOutletDesignTemperature(receiverTemp),
  _storageTankHeight(storageHeight),
  _storageTankDiameter(storageDiameter),
  _storageInsulationThickness(insulationThickness),
  _steamGeneratorOutletTemperature(exchangerExitTemp),
  _receiverApertureHeight(apertureHeight),
  _receiverApertureWidth(apertureWidth),

  //Simulation parameters
  _iterationPrecision(precision),
  _timeInterval(timeInterval)
{ 
	_steamGenOutletMsRate.reserve(24 * _timeInterval * 60);
	_steamGenOutletTemp.reserve(24 * _timeInterval * 60);
	_storageHeat.reserve(24 * _timeInterval * 60);
...
HtfCycle::HtfCycle(double receiverTemp,
		   ...
		   double receiverTubesDin, double receiverTubesThickness, int receiverNbTubes,
		   double precision, int timeInterval,
		   double exchangerTubesDin, double exchangerTubesDout, double exchangerTubesLength, double exchangerTubesSpacing,
		   double baffleCut, int nbOfBaffles,
		   int exchangerNbOfTubes, int exchangerNbOfPassesPerShell, int exchangerNbOfShells)
:
  //Mechanical components
  _centralReceiver(&_centralReceiverInlet, &_centralReceiverOutlet, apertureHeight, apertureWidth, insulationThickness, receiverTubesDin, receiverTubesThickness, receiverNbTubes),
  _hotStorage(&_centralReceiverOutlet, &_steamGeneratorInlet, storageHeight, storageDiameter, insulationThickness),
  _coldStorage(&_steamGeneratorOutlet, &_centralReceiverInlet, 1.2*storageHeight, storageDiameter, insulationThickness),
  _steamGenerator(&_steamGeneratorInlet, &_steamGeneratorOutlet, powerblock,
		  exchangerTubesLength, exchangerTubesDin, exchangerTubesDout, exchangerTubesSpacing,
		  baffleCut, nbOfBaffles,
		  exchangerNbOfTubes, exchangerNbOfPassesPerShell, exchangerNbOfShells),
...
void HtfCycle::fOperateCycle(int timeInSeconds, double energyFromField, double requiredPowerOutput /*W*/)
{
	std::logic_error casNotSet("Could not start interation with a valid value for cas");
	...
	double availableMass, inputMass, outputMass;
	double hotMassFlow_in, hotMassFlow_out, hotMassFlow_out2;
	double hotTemp_in, hotTemp_out, hotTemp_out2;
	double heatedMass;
	double temperaturePrecision = 0.01 / timeInSeconds;
	unsigned int cas, count;
	int steamGeneratorModel = _steamGenerator.get_exchangerModel();
	double Cp = HEAT_CAPACITY;
	double minimumHotMass = 0.05*MS_DENSITY*_hotStorage.get_heightOfStorage()*M_PI*pow(_hotStorage.get_diameterOfStorage() / 2., 2.);

	//-----------------------------------------------------------------------------
	//Determiner les conditions de sortie du récepteur
	...
	if (hotMassFlow_in == 0. && hotMass_i == 0.){ cas = 1; }
	else if (hotMassFlow_in == 0. && hotMass_i > 0.) { cas = 2; }
	else if (hotMassFlow_in > 0. && hotMass_i == 0.) { cas = 3; }
	else if (hotMassFlow_in > 0. && hotMass_i > 0.)  { cas = 4; }

	else if (hotMassFlow_in == 0. && hotMass_i < 0.) { hotMass_i = 0.; cas = 1; }
	else if (hotMassFlow_in >0. && hotMass_i < 0.) { hotMass_i = 0.; cas = 3; }
	else { 
		throw casNotSet;
		std::cout << "hotMassFlow_in = " << hotMassFlow_in << "   hotMass_i = " << hotMass_i << std::endl;}

	switch (cas)
	{

		//Le storage est vide et on ne recolte pas d'energie du recepteur
		//=========================================================================================
	case 1:
		hotTemp_i = 0;
		hotLevel_i = 0.;
		...
		//Le storage n'est pas vide et aucune energie n'est recoltee du recepteur
		//=========================================================================================
	case 2:
		heatedMass = 0.;
		...
		if (steamGeneratorModel == 1){
			...
			hotMassFlow_out = _steamGenerator.fComputeRequiredMoltenSaltMassFlow(requiredPowerOutput, hotTemp_i, availableMass / timeInSeconds);
		}
		...
		//Si on a suffisament de masse a piori, on determine les conditions finales approximatives
		//Sinon, Le cycle sera en arret et on calcule seulement les pertes thermiques du storage et on sort
			...
			|| hotMassFlow_out == 0.)
		{
			...
			hotTemp_avg = (hotTemp_i + hotTemp_f) / 2.;
			hotTemp_avg2 = 0.;
			//IF U_F < MELTING POINT, THROW EXCEPTION AND BREAK
			count = 0;
			while (fabs(hotTemp_avg2 - hotTemp_avg) > 0.001
				&& count < 15)
			{
			...
			hotMass_avg = (hotMass_i + hotMass_f) / 2.;
			...
			else{ hotMassFlow_out = _steamGenerator.fComputeRequiredMoltenSaltMassFlow(requiredPowerOutput, hotTemp_avg, availableMass / timeInSeconds); }
			count = 0;
				...
				&& count < 15)
			{
				...
				hotMass_f = hotMass_i - timeInSeconds*hotMassFlow_out;
				hotMass_avg = (hotMass_f + hotMass_i) / 2.;
				hotLevel_avg = _hotStorage.fComputeStorageLevel(hotMass_avg);
				...
				if (steamGeneratorModel == 1){ hotMassFlow_out = _steamGenerator.fComputeRequiredMoltenSaltMassFlow(requiredPowerOutput, hotTemp_avg); }
				else{ hotMassFlow_out = _steamGenerator.fComputeRequiredMoltenSaltMassFlow(requiredPowerOutput, hotTemp_avg, availableMass / timeInSeconds); }
				++count;
			}
		...
		//Le storage est initialement vide et on recoit de l'energie du recepteur
		//=========================================================================================
	case 3:
		...
		//Si les requis du powerblock sont superieurs a l'input, on verifiera si on peut operer le cycle avec un input moindre
		//mais produire moins d'energie ou produire a un rendement inferieur. aucune perte n'est consideree
		if (timeInSeconds*hotMassFlow_in < minimumHotMass)
		{
			...
			if (timeInSeconds*hotMassFlow_out > availableMass){ /*hotMassFlow_out = availableMass / timeInSeconds;*/
				hotMassFlow_out = 0.;
			}
			hotMass_f = timeInSeconds*(hotMassFlow_in - hotMassFlow_out);
			hotMass_avg = hotMass_f / 2.;
			hotLevel_avg = _hotStorage.fComputeStorageLevel(hotMass_avg);
		...
		}else if (hotLevel_f > _hotStorage.get_heightOfStorage() && hotMassFlow_out == 0.)
		{
			hotLevel_f = _hotStorage.get_heightOfStorage();
			hotMass_f = MS_DENSITY*hotLevel_f * M_PI*pow(_hotStorage.get_diameterOfStorage(), 2.) / 4.;
			...
			_hotStorage.set_storage(hotMass_f, hotTemp_f);
			_steamGeneratorInlet.set_massFlow(0.);
			_steamGeneratorOutlet.set_massFlow(0.);
			break;
		}
		...
		//Le storage n'est pas vide et recoit un debit chaud en provenance du recepteur
		//=========================================================================================
	case 4:
		hotMass_i = _hotStorage.get_storedMass();
		hotTemp_i = _hotStorage.get_storedTemperature();
		...
		if (/*availableMass < hotMassFlow_out*timeInSeconds*/
			availableMass - timeInSeconds*hotMassFlow_out < minimumHotMass)
		{
			/*hotMassFlow_out = availableMass / timeInSeconds;*/
			if (availableMass > minimumHotMass)
			{
				hotMassFlow_out = /*(availableMass - minimumHotMass) / timeInSeconds;*/ 0.;
		...
		//Si le storage devient plein au cours de l'interval
		if (hotLevel_f > _hotStorage.get_heightOfStorage())
		{
			...
			if (steamGeneratorModel == 1)
			{
				...
				hotMassFlow_out = _steamGenerator.fComputeRequiredMoltenSaltMassFlow(requiredPowerOutput, hotTemp_avg, availableMass / timeInSeconds);
			}
			hotTemp_avg2 = 0.;
			hotMassFlow_out2 = 0.;
			count = 0;
	...
	double coldLevl_i, coldLevl_f, coldLevl_avg;
	...
	coldMass_avg = (coldMass_i + coldMass_f) / 2.;
	...
	coldTemp_avg = (coldTemp_i + coldTemp_f) / 2.;
	coldTemp_avg2 = 0.;

	count = 0;
	while ( fabs(coldTemp_avg2 - coldTemp_avg) > temperaturePrecision
		&& count < 15)
	{
	...
	maxColdMass = _coldStorage.get_heightOfStorage() *
		M_PI * pow(_coldStorage.get_diameterOfStorage()/2.,2.) * MS_DENSITY;
...
void HtfCycle::initiateColdStorage()
{
	double volume = M_PI*pow(_coldStorage.get_diameterOfStorage() / 2., 2.)*_coldStorage.get_heightOfStorage();
...
void HtfCycle::setStorage(double percentHot, double tempHot, double tempCold)
{
	...
	_hotStorage.set_storage2(H*(percentHot / 100.), tempHot);
	_coldStorage.set_storage2(cold_H - (H*percentHot / 100.), tempCold);
}
